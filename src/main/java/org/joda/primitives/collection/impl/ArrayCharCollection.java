...
public class ArrayCharCollection extends AbstractCharCollection implements Cloneable {
     ...
     */
    public ArrayCharCollection(Iterator it) {
        super();
        if (it == null) {
            iData = CharUtils.EMPTY_CHAR_ARRAY;
        } else if (it instanceof CharIterator) {
            CharIterator typed = (CharIterator) it;
            iData = new char[MIN_GROWTH_SIZE];
            while (typed.hasNext()) {
                add(typed.nextChar());
            }
        } else {
    ...
    public boolean addAll(CharCollection values) {
        ...
        values.toCharArray(iData, iSize);
        iSize += len;
        return true;
    }
    ...
    protected static class PIterator implements CharIterator {
        ...
        private final ArrayCharCollection iCollection;
        ...
        public Object next() {
            return iCollection.toObject(nextChar());
        }
