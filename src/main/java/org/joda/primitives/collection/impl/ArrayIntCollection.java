...
public class ArrayIntCollection extends AbstractIntCollection implements Cloneable {
    ...
    public ArrayIntCollection() {
        super();
        iData = IntUtils.EMPTY_INT_ARRAY;
    }
    ...
    public ArrayIntCollection(Collection coll) {
        ...
        if (coll == null) {
            ...
            IntCollection c = (IntCollection) coll;
            iSize = c.size();
            iData = new int[iSize];
            c.toIntArray(iData, 0);
        } else {
     ...
     */
    public ArrayIntCollection(Iterator it) {
        super();
        if (it == null) {
            iData = IntUtils.EMPTY_INT_ARRAY;
        } else if (it instanceof IntIterator) {
            IntIterator typed = (IntIterator) it;
            iData = new int[MIN_GROWTH_SIZE];
            while (typed.hasNext()) {
                add(typed.nextInt());
            }
        } else {
     ...
     */
    public Object clone() {
        ArrayIntCollection cloned = (ArrayIntCollection) super.clone();
        cloned.iData = (int[]) iData.clone();
        return cloned;
    }
    ...
    protected static class PIterator implements IntIterator {
        ...
        private final ArrayIntCollection iCollection;
        ...
        public Object next() {
            return iCollection.toObject(nextInt());
        }
